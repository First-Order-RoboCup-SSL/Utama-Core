...
from utama_core.entities.data.vision import VisionBallData
...
class DribbleToTarget:
    ...
    def enact(self, has_ball: bool):
        ...
        self._update_dribble_distance((current_x, current_y), has_ball)  # update distance traveled with ball
        ...
        if distance((current_x, current_y), self.target_coords) <= self.tolerance:
            ...
            if not has_ball:
                ...
                if self.augment:
                    ...
                    ball_data = VisionBallData(0, delta_x * 6 + current_x, delta_y * 6 + current_y, 0)
                return go_to_ball(
                    self.pid_oren,
                    self.pid_trans,
                    this_robot_data,
    ...
    def _update_dribble_distance(self, current_point: tuple[float, float], has_ball: bool):
