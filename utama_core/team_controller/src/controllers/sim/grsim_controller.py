...
from utama_core.team_controller.src.generated_code.ssl_gc_common_pb2 import (
    RobotId,
    Team,
)
from utama_core.team_controller.src.generated_code.ssl_simulation_control_pb2 import (
    SimulatorCommand,
    SimulatorControl,
    TeleportBall,
    TeleportRobot,
...
class GRSimController(AbstractSimController):
    ...
    def teleport_ball(self, x: float, y: float, vx: float = 0, vy: float = 0) -> None:
        ...
        time.sleep(0.05)  # Allow some time for the command to be processed
        self.net.send_command(sim_command)
        time.sleep(0.05)
    ...
    def teleport_robot(
        ...
        This method creates a command for teleporting the ball and sends it to the simulator.
        """
        sim_control = self._create_teleport_robot_command(robot_id, is_team_yellow, x, y, theta)
        sim_command = self._create_simulator_command(sim_control)
        self.net.send_command(sim_command)
    ...
    def _get_teleport_location(self, robot_id: int, is_team_yellow: bool, add: bool) -> Tuple[float, float]:
        ...
        x_coord = -TELEPORT_X_COORDS[robot_id] if is_team_yellow else TELEPORT_X_COORDS[robot_id]
