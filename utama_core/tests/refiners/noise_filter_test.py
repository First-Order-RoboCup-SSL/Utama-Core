...
class RobotFilterTest:
    def __init__(self, id: int):
        ...
        self.baseline_x = RobotFilterTest._mean_squared_error(self._clean, self._noisy, X_COL)

        self.baseline_y = RobotFilterTest._mean_squared_error(self._clean, self._noisy, Y_COL)

        self.baseline_v = RobotFilterTest._mean_squared_error_vec(self._clean, self._noisy)
        ...
        for _, entry in self._noisy.iterrows():
            ...
            new_row = pd.DataFrame({X_COL: x_f, Y_COL: y_f, TH_COL: th_f}, index=[0])
            self._filtered = pd.concat([self._filtered, new_row], ignore_index=True)

        self._filtered = self._filtered.reset_index(drop=True)

        self.error_x = RobotFilterTest._mean_squared_error(self._clean, self._filtered, X_COL)

        self.error_y = RobotFilterTest._mean_squared_error(self._clean, self._filtered, Y_COL)

        self.error_v = RobotFilterTest._mean_squared_error_vec(self._clean, self._filtered)

    @staticmethod
    def _mean_squared_error(true_data: pd.DataFrame, actual_data: pd.DataFrame, param: str) -> float:
        return true_data[param].combine(other=actual_data[param], func=lambda t, a: (t - a) ** 2).mean()
    ...
    @staticmethod
    def _mean_squared_error_vec(true_data: pd.DataFrame, actual_data: pd.DataFrame) -> float:

        true_vec = true_data[X_COL].combine(other=true_data[Y_COL], func=RobotFilterTest.vectorify)

        actual_vec = actual_data[X_COL].combine(other=actual_data[Y_COL], func=RobotFilterTest.vectorify)
