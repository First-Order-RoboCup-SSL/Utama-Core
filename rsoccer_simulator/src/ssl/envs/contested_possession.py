...
class SSLContestedPossessionEnv(SSLBaseEnv):
    ...
    def _get_initial_positions_frame(self):
        ...
        def x():
            return random.uniform(pen_len, half_len - pen_len)

        def y():
            return random.uniform(-half_pen_wid, half_pen_wid)

        pos_frame.robots_blue[0] = Robot(x=0, y=0, theta=0.0)
        enemy_x = x()
        enemy_y = y()

        pos_frame.ball = Ball(x=enemy_x - 0.1, y=enemy_y)
        pos_frame.robots_yellow[0] = Robot(x=enemy_x, y=enemy_y, theta=180.0)
