...
class SSLHWDribblingEnv(SSLBaseEnv):
        ...
        if robot_out_of_bounds(robot):
            ...
            if self.checkpoints_count == 0:
                ...
                if self.checkpoints_count % 2 == 0:
                    ...
                    if (
                        ball.x > self.node_3 - self.field_margin
                        and ball.x < self.node_3
                    ):
                        if last_ball.y < 0 and ball.y >= 0:
                            reward = 1
                            self.checkpoints_count += 1
    ...
    def _get_initial_positions_frame(self):
        ...
        pos_frame.ball = Ball(x=-0.1, y=0.0)

        pos_frame.robots_blue[0] = Robot(x=0.0, y=0.0, theta=180.0)

        pos_frame.robots_yellow[0] = Robot(x=self.node_0, y=0.0, theta=180.0)
        pos_frame.robots_yellow[1] = Robot(x=self.node_1, y=0.0, theta=180.0)
        pos_frame.robots_yellow[2] = Robot(x=self.node_2, y=0.0, theta=180.0)
        pos_frame.robots_yellow[3] = Robot(x=self.node_3, y=0.0, theta=180.0)
