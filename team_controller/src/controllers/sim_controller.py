...
from config.settings import LOCAL_HOST, SIM_COMTROL_PORT, TELEPORT_X_COORDS, FIELD_Y_COORD, REMOVAL_Y_COORD

from generated_code.ssl_simulation_control_pb2 import TeleportBall, TeleportRobot, SimulatorControl, SimulatorCommand
from generated_code.ssl_gc_common_pb2 import RobotId, Team
...
class SimulatorController:
    ...
    def set_robot_presence(self, robot_id: int, team_colour_is_blue: bool, should_robot_be_present: bool) -> None:
        """
        ...
        """
        x, y = self._get_teleport_location(robot_id, team_colour_is_blue, should_robot_be_present)
        sim_control = self._create_teleport_robot_command(robot_id, team_colour_is_blue, x, y, should_robot_be_present)
        sim_command = self._create_simulator_command(sim_control)
        self.net.send_command(sim_command)

    def _create_teleport_robot_command(self, robot_id: int, team_colour_is_blue: bool, x: float, y: float, present: bool) -> object:
        robot = RobotId(id=robot_id, team=Team.BLUE if team_colour_is_blue else Team.YELLOW)
        tele_robot = TeleportRobot(id=robot, x=x/1000, y=y/1000, present=present)
        sim_control = SimulatorControl()
        sim_control.teleport_robot.add().CopyFrom(tele_robot)
        return sim_control

    def _get_teleport_location(self, robot_id: int, team_colour_is_blue: bool, add: bool) -> Tuple[float, float]:
        y_coord = FIELD_Y_COORD if add else REMOVAL_Y_COORD
        x_coord = -TELEPORT_X_COORDS[robot_id] if team_colour_is_blue else TELEPORT_X_COORDS[robot_id]
