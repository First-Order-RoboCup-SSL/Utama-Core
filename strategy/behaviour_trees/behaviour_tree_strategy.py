...
class BehaviourTreeStrategy(Strategy):
    def __init__(self, robot_controller: AbstractRobotController, behaviour: py_trees.behaviour.Behaviour):
        ...
        self.blackboard.register_key(key="present_future_game", access=py_trees.common.Access.EXCLUSIVE_WRITE)
    ...
    def step(self, present_future_game: PresentFutureGame):
        ...
        end_time = time.time()
        logger.info("Behaviour Tree %s executed in %f secs", self.behaviour_tree.__class__.__name__, end_time - start_time)
