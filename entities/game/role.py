...
class Role:
    ...
    def change_role(self, role: Union[int, str]) -> None:
        if isinstance(role, int):
            if 0 <= role < len(ROLES):
                ...
                raise ValueError(f"Invalid role index: {role}")
        elif isinstance(role, str) and role in ROLES:
            self.role = role
        else:
    ...
    def _get_actions_for_role(self) -> List[Action]:
        role_actions = {
            "attack": [
                Action(action="pass"), Action(action="shoot_to_goal"), Action(action="go_to_ball"), 
                Action(action="cross_ball"), Action(action="receive_then_score")
            ],
            "defend": [
                Action(action="pass"), Action(action="man_mark"), Action(action="intercept_ball"), 
                Action(action="tackle ball"), Action(action="clear ball"), Action(action="block"), 
                Action(action="go_to_ball"), Action(action="receive_ball")
            ]
        }
