...
def test_passing_yellow_game_initial_frame_is_reflected_in_robots():
    ...
    for i in range(6):
        ...
        assert game.friendly_robots[i].x == 100*i, f"Expected {100*i}, got {game.friendly_robots[i].x}"
        assert game.friendly_robots[i].y == 1000*i, f"Expected {1000*i}, got {game.friendly_robots[i].y}"
        assert game.friendly_robots[i].orientation == 0

        assert game.enemy_robots[i].x == i, f"Expected {i}, got {game.enemy_robots[i].x}"
        assert game.enemy_robots[i].y == 10*i, f"Expected {10*i}, got {game.enemy_robots[i].y}"
        assert game.enemy_robots[i].orientation == 0

def test_passing_blue_game_initial_frame_is_reflected_in_robots():
    game = Game(False, True, start_frame=FrameData(0, yellow_robot_data, blue_robot_data, [basic_ball]))
    ...
    for i in range(6):
        ...
        assert game.friendly_robots[i].x == i, f"Expected {i}, got {game.friendly_robots[i].x}"
        assert game.friendly_robots[i].y == 10*i, f"Expected {10*i}, got {game.friendly_robots[i].y}"
        assert game.friendly_robots[i].orientation == 0

        assert game.enemy_robots[i].x == 100*i, f"Expected {i*100}, got {game.enemy_robots[i].x}"
        assert game.enemy_robots[i].y == 1000*i, f"Expected {1000*i}, got {game.enemy_robots[i].y}"
        assert game.enemy_robots[i].orientation == 0
