...
if __name__ == "__main__":
    ...
    pid_oren = PID(TIMESTEP, 1, -1, 0.5, 0.0, 0.0, num_robots=6)
    ...
    sim_robot_controller_yellow = RSimRobotController(
        is_team_yellow=False, env=env, game_obj=game, debug=True
    )
    ...
    try:
        ...
        while True:
            ...
            latest_frame = game.get_my_latest_frame()
            if latest_frame:
                friendly_robots, enemy_robots, balls = latest_frame
                friendly_robots = enemy_robots
    ...
    ty - cy, tx - cx
                ...
                if math.dist((tx, ty), (cx, cy))  < 0.02:
                    ty *= -1
                from entities.data.command import RobotCommand
                import math
                oren =  math.pi if ty > 0 else -math.pi
                cmd = go_to_point(pid_oren, pid_2d, friendly_robots[defender_ids[0]], defender_ids[0], (tx, ty), None)
                # raw = RobotCommand(local_forward_vel=-0.5, local_left_vel=0, angular_vel=0, kick_angle=0, kick_spd=0, dribbler_spd=0)
