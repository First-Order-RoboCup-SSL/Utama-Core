...
def test_2v5_play(friendly_robot_ids: List[int], is_yellow: bool, headless: bool):
    ...
    player3_id = friendly_robot_ids[2]
    player4_id = friendly_robot_ids[3]
    ...
    try:
        while True:
            ...
            if not scored:
                ...
                player3_has_ball = sim_robot_controller.robot_has_ball(player3_id)
                player4_has_ball = sim_robot_controller.robot_has_ball(player4_id)
                ...
                elif player3_has_ball:
                    ball_possesor_id = player3_id
                elif player4_has_ball:
                    ball_possesor_id = player4_id
                ...
                (
                    sim_robot_controller.add_robot_commands(
                        commands.get(player3_id), player3_id
                    )
                    if commands.get(player3_id)
                    else None
                )
                (
                    sim_robot_controller.add_robot_commands(
                        commands.get(player4_id), player4_id
                    )
                    if commands.get(player4_id)
                    else None
                )
...
if __name__ == "__main__":
    test_2v5_play([0, 1, 2, 3], False, False)
