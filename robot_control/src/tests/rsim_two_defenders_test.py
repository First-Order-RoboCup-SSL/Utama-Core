...
def defend(pid_oren:PID, pid_2d: TwoDPID, game: Game, controller: RSimRobotController, is_yellow: bool, defender_ids: List[int], env):
    ...
    cmd = go_to_point(pid_oren, pid_2d, friendly[defender1_id], defender1_id, target, face_ball(real_def_pos, (balls[0].x, balls[0].y)))
...
def test_two_defenders(defender_id: int, shooter_id: int, defender_is_yellow: bool, headless: bool):
    ...
    for _ in range(900):
        ...
        if scored:
            any_scored = True
            break
        defend(pid_oren_d, pid_2d_d, game, sim_robot_controller_defender, defender_is_yellow, [defender_id, -1], env)
