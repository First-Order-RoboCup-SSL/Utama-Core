...
def test_shooting(shooter_id: int, defender_is_yellow: bool, headless: bool):
    ...
    shoot_in_left_goal = not defender_is_yellow  # Need to get this from ref info in real life; in unit testing this is fine
    goal_scored = False
    ...
    for iter in range(ITERS):
        ...
        if not goal_scored:
            ...
            if target and not find_likely_enemy_shooter(enemy, balls):
                cmd = go_to_point(pid_oren_d, pid_2d_d, friendly[0], 0, target, face_ball((friendly[0].x, friendly[0].y), game.get_ball_pos()[0]))
