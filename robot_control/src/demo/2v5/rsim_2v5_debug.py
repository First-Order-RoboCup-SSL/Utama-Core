...
def test_rsim_2v5():
    ...
    try:
        pass_task0 = PassBall(pid_oren, pid_trans, game, r0, r1, receiving_points[0])
        # r0 get ball
        while distance((game.ball.x, game.ball.y), target_ball_pos) < 0.1:
            env.draw_point(receiving_points[0][0], receiving_points[0][1], width=2)
            cmd0, cmd1 = pass_task0.enact(False)
        ...
        target_oren = np.arctan2(
            receiving_points[0][1] - passing_points[0][1],
            receiving_points[0][0] - passing_points[0][0],
        )
        while True:
            env.draw_point(
                passing_points[0][0], passing_points[0][1], width=2, color="yellow"
            )
            r0_data = game.get_robot_pos(True, r0)
            if distance((r0_data.x, r0_data.y), passing_points[0]) < 0.05:
                break
            cmd = go_to_point(
                pid_oren, pid_trans, r0_data, r0, passing_points[0], target_oren, True
            )
            sim_robot_controller.add_robot_commands(cmd, r0)
            sim_robot_controller.send_robot_commands()

        # r0 pass ball
        start_t = time.time()
        while time.time() - start_t < 3:
            cmd0, cmd1 = pass_task0.enact(True)
            sim_robot_controller.add_robot_commands(cmd0, r0)
            sim_robot_controller.add_robot_commands(cmd1, r1)
            sim_robot_controller.send_robot_commands()
        ...
        while True:
            ...
            r0_data = game.get_robot_pos(True, r0)
            if distance((r1_data.x, r1_data.y), passing_points[1]) < 0.05:
                break
            cmd0 = go_to_point(
                pid_oren,
                pid_trans,
