...
def test_pathfinding(headless: bool, moving: bool):
    ...
    hybrid = HybridWaypointMotionController(N_ROBOTS_YELLOW, game, env)
    targets = [(0, 0)] + [
    ...
    for i in range(5000):
        ...
        wayps = hybrid._intermediate_targets[mover_id]

        for x in hybrid._waypoint_producer.par.keys():
            if hybrid._waypoint_producer.par[x] is not None:
                env.draw_line(
                    [point_to_tuple(x), point_to_tuple(hybrid._waypoint_producer.par[x])],
                    color="BLUE",
                    width=3,
                )
        if wayps:
            for i in range(1, len(wayps)):
                env.draw_line([wayps[i - 1], wayps[i]], color="GREEN", width=2)
        else:
            env.draw_line([next_stop, (r.x, r.y)], color="PINK", width=2)
