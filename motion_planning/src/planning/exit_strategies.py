...
class ExitStrategy(ABC):
    ...
    @abstractmethod
    def get_exit_point(self, robot_position: Tuple[float, float], obstacles: List[Polygon]) -> Optional[Tuple[float, float]]:
        ...
    ...
    def _is_too_close(self, robot_position:Tuple[float, float], obstacles: List[Polygon]) -> bool:
        ...
        for obstacle in obstacles:
            if obstacle.contains(robot_point) or obstacle.distance(robot_point) < self.OBSTACLE_SAFE_BUFFER:
                return True
        return False
    ...
    def closest_point_with_buffer(polygon: Polygon, robot_pos: Point) -> Point:
        ...
        closest_point = polygon.exterior.interpolate(polygon.exterior.project(robot_pos))
        ...
        if polygon.contains(closest_point):
            ...
            print("INSIDE")
            direction_vec = (robot_pos.x - closest_point.x, robot_pos.y - closest_point.y)


        else:
            print("OUTSIDE", robot_pos, closest_point)
            # Outside case: Move further out (robot → closest point, reversed)
            direction_vec = (closest_point.x - robot_pos.x, closest_point.y - robot_pos.y)
        ...
        if direction_length > 0:
            unit_vec = (direction_vec[0] / direction_length, direction_vec[1] / direction_length)
        else:
    ...
    def is_close_enough_to_exit_point(robot_pos: Tuple[float, float], exit_point: Tuple[float, float]) -> bool:
        ...
        """
        return Point(robot_pos).distance(Point(exit_point)) < ExitStrategy.CLOSE_ENOUGH_TO_EXIT_POINT
